...
func (e *Api) MakeRuntime() *Api {
	...
	if run == nil {
		e.Logger.Error(http.StatusInternalServerError, err, "runtime初始化失败")
		e.AddError(err)
	}
	e.Run = run
	return e
...
func (e *Api) getAcceptLanguage() string {
	languages := lang.ParseAcceptLanguage(e.Context.GetHeader("Accept-Language"), nil)
	if len(languages) == 0 {
