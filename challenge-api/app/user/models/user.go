...
type AppUser struct {
	...
	Username    string          `gorm:"column:username;type:varchar(100);not null;default:'';uniqueIndex:uk_username;comment:账号名称/用户名" json:"username"`
	...
	Email       string          `gorm:"column:email;type:varchar(300);uniqueIndex:uk_email;comment:电子邮箱" json:"email"`
	MobileTitle string          `gorm:"column:mobile_title;type:varchar(255);default:'+86';uniqueIndex:uk_mobile,priority:1;comment:用户手机号国家前缀" json:"mobile_title"`
	Mobile      string          `gorm:"column:mobile;type:varchar(100);uniqueIndex:uk_mobile,priority:2;comment:手机号码" json:"mobile"`
	...
	RefCode     string          `gorm:"column:ref_code;type:varchar(255);uniqueIndex:uk_ref_code;comment:推荐码" json:"ref_code"`
	ParentID    int             `gorm:"column:parent_id;not null;default:0;index:idx_parent_id;comment:父级编号" json:"parent_id"`
	...
	RegisterAt  time.Time       `gorm:"column:register_at;type:datetime;not null;default:CURRENT_TIMESTAMP;index:idx_register_at;comment:注册时间" json:"register_at"`
	RegisterIp  string          `gorm:"column:register_ip;type:varchar(45);comment:注册IP" json:"register_ip"`
	LastLoginAt *time.Time      `gorm:"column:last_login_at;type:datetime;index:idx_last_login_at;comment:最后登录时间" json:"last_login_at"`
