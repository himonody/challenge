...
type UserAccountLog struct {
	...
	UserId      int64           `json:"userId" gorm:"column:user_id;type:int;index:idx_user_id_created,priority:1;comment:用户编号"`
	...
	ChangeType  string          `json:"changeType" gorm:"column:change_type;type:varchar(30);index:idx_change_type;comment:帐变类型(1-类型1)"`
	Status      string          `json:"status" gorm:"column:status;type:char(1);index:idx_status;comment:状态（1正常 2-异常）"`
	CreateBy    int64           `json:"createBy" gorm:"column:create_by;type:int;comment:创建者"`
	CreatedAt   *time.Time      `json:"createdAt" gorm:"column:created_at;type:datetime;index:idx_user_id_created,priority:2;comment:创建时间"`
