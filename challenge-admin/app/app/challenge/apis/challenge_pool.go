...
func (e ChallengePool) ChallengePoolPage(c *gin.Context) {
	req := dto.ChallengePoolQueryReq{}
	s := service.ChallengePool{}
...
func (e ChallengePool) ChallengePoolExport(c *gin.Context) {
	req := dto.ChallengePoolQueryReq{}
	s := service.ChallengePool{}
	err := e.MakeContext(c).MakeOrm().Bind(&req).MakeService(&s.Service).Errors
	if err != nil {
	...
	data, _ := s.ExportChallengePool(list)
	fileName := "challenge_pool_" + dateutils.ConvertToStr(time.Now(), 3) + ".xlsx"
	e.DownloadExcel(fileName, data)
}

func (e ChallengePool) ChallengePoolCreate(c *gin.Context) {
	req := dto.ChallengePoolCreateReq{}
	s := service.ChallengePool{}
	err := e.MakeContext(c).MakeOrm().Bind(&req).MakeService(&s.Service).Errors
	if err != nil {
		e.Error(baseLang.DataDecodeCode, lang.MsgLogErrf(e.Logger, e.Lang, baseLang.DataDecodeCode, baseLang.DataDecodeLogCode, err).Error())
...
func (e ChallengePool) ChallengePoolUpdate(c *gin.Context) {
	req := dto.ChallengePoolUpdateReq{}
	s := service.ChallengePool{}
	err := e.MakeContext(c).MakeOrm().Bind(&req).MakeService(&s.Service).Errors
	if err != nil {
		e.Error(baseLang.DataDecodeCode, lang.MsgLogErrf(e.Logger, e.Lang, baseLang.DataDecodeCode, baseLang.DataDecodeLogCode, err).Error())
...
func (e ChallengePool) ChallengePoolDelete(c *gin.Context) {
	var uri struct {
		Id uint64 `uri:"id"`
	}
	s := service.ChallengePool{}
