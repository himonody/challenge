...
func (e ChallengeCheckin) CheckinPage(c *gin.Context) {
	req := dto.ChallengeCheckinQueryReq{}
	s := service.ChallengeCheckin{}
...
func (e ChallengeCheckin) CheckinExport(c *gin.Context) {
	req := dto.ChallengeCheckinQueryReq{}
	s := service.ChallengeCheckin{}
	err := e.MakeContext(c).MakeOrm().Bind(&req).MakeService(&s.Service).Errors
	if err != nil {
	...
	data, _ := s.ExportCheckin(list)
	fileName := "challenge_checkin_" + dateutils.ConvertToStr(time.Now(), 3) + ".xlsx"
	e.DownloadExcel(fileName, data)
}

func (e ChallengeCheckin) CheckinCreate(c *gin.Context) {
	req := dto.ChallengeCheckinCreateReq{}
	s := service.ChallengeCheckin{}
	err := e.MakeContext(c).MakeOrm().Bind(&req).MakeService(&s.Service).Errors
	if err != nil {
		e.Error(baseLang.DataDecodeCode, lang.MsgLogErrf(e.Logger, e.Lang, baseLang.DataDecodeCode, baseLang.DataDecodeLogCode, err).Error())
...
func (e ChallengeCheckin) CheckinUpdate(c *gin.Context) {
	req := dto.ChallengeCheckinUpdateReq{}
	s := service.ChallengeCheckin{}
	err := e.MakeContext(c).MakeOrm().Bind(&req).MakeService(&s.Service).Errors
	if err != nil {
		e.Error(baseLang.DataDecodeCode, lang.MsgLogErrf(e.Logger, e.Lang, baseLang.DataDecodeCode, baseLang.DataDecodeLogCode, err).Error())
...
func (e ChallengeCheckin) CheckinDelete(c *gin.Context) {
	var uri struct {
		Id uint64 `uri:"id"`
	}
	s := service.ChallengeCheckin{}
