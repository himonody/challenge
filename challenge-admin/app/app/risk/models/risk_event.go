...
type RiskEvent struct {
	...
	EventType int8       `json:"eventType" gorm:"column:event_type;type:tinyint;comment:事件类型"`
	Detail    string     `json:"detail" gorm:"column:detail;type:varchar(255);comment:事件详情"`
	Score     int        `json:"score" gorm:"column:score;type:int;comment:风险分"`
	CreatedAt *time.Time `json:"createdAt" gorm:"column:created_at;type:datetime;comment:发生时间"`
}

func (RiskEvent) TableName() string { return "app_risk_event" }
