...
import {
	ChallengeUserModel,
	getChallengeUserPageApi,
	exportChallengeUserApi,
	delChallengeUserApi
} from "@/api/app/challenge/user";
import type { ActionType } from "@ant-design/pro-components";
import FormModal from "./components/FormModal";
import type { FormModalRef } from "./components/FormModal";

const ChallengeUserPage: React.FC = () => {
	...
	const handleExport = (done: () => void) => {
		modal.confirm({
			...
			onOk: async () => {
				try {
					saveExcelBlob("challenge_user", await exportChallengeUserApi(formRef.current?.getFieldsValue()));
				} catch (err) {
					message.error("导出失败");
				} finally {
					done();
	...
	const columns: ProColumns<ChallengeUserModel>[] = [
		...
		{ title: "失败原因", dataIndex: "failReason", width: 100, hideInSearch: true },
	...
	const toolBarRender = () => [
		<HocAuth key="add" permission={["app:challenge:user:add"]}>
			<LoadingButton type="primary" key="addBtn" icon={<PlusCircleOutlined />} onClick={() => handleShowCreate()}>
				新增
			</LoadingButton>
		</HocAuth>,
		<HocAuth key="export" permission={["app:challenge:user:export"]}>
		...
		<>
			<ProTable<ChallengeUserModel>
				...
				request={async params => {
					const { data } = await getChallengeUserPageApi(params);
					return formatDataForProTable<ChallengeUserModel>(data);
				}}
